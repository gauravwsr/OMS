...
const HRLeaveManagement = () => {
  ...
  const fetchLeaveApplications = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://138.197.27.240:5001/api/leave/employees');
  ...
  const handleLeaveAction = async (leaveId, action, comments = '') => {
    try {
      ...
      console.log('Making API call to:', `http://138.197.27.240:5001/api/leave/status/${leaveId}`);

      const response = await axios.patch(`http://138.197.27.240:5001/api/leave/status/${leaveId}`, requestData, {
        headers: {
