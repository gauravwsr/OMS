...
const EmailConfig = ({ onConfigured, isModal = false }) => {
  ...
  const checkEmailConfiguration = async () => {
    ...
    try {
      ...
      if (response.ok && data.configured) {
        ...
        // Don't auto-redirect when checking config on page load
  ...
  const saveEmailConfiguration = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setTestStatus({ type: 'success', message: 'Email configured successfully! You can now go to your inbox.' });
        setIsConfigured(true);
        // Don't auto-redirect after saving, let user manually navigate
        // setTimeout(() => {
        //   if (onConfigured) {
        //     onConfigured();
        //   }
