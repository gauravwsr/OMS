...
const Todo = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://138.197.27.240:5001/tasks?userEmail=${encodeURIComponent(
          user.email
        )}`
      )
      .then((res) => {
    ...
    const fetchEmails = async () => {
      try {
        ...
        const usersResponse = await axios.get(
          "http://138.197.27.240:5001/api/users"
        );
  ...
  const addTask = () => {
    ...
    axios
      .post("http://138.197.27.240:5001/tasks", task)
      .then((res) => {
  ...
  const toggleTask = (id) => {
    ...
    axios
      .put(`http://138.197.27.240:5001/tasks/${id}`, { completed: !task.completed })
      .then(() => {
        setTasks(
          tasks.map((t) =>
  ...
  const deleteTask = (id) => {
    axios
      .delete(`http://138.197.27.240:5001/tasks/${id}`)
      .then(() => {
