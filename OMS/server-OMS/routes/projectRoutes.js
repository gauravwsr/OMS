  ...
  try {
    ...
    for (const remote of projectsData) {
      ...
      const update = {
        ...
        tasks: remote.tasks ? [
          { name: 'Completed', status: 'completed', dueDate: null, completed: remote.tasks.completed >= 1 },
          { name: 'In Progress', status: 'inProgress', dueDate: null, completed: remote.tasks.inProgress >= 1 },
          { name: 'Pending', status: 'pending', dueDate: null, completed: remote.tasks.pending >= 1 }
        ] : [],
    ...
    res.status(200).json({ message: 'Projects imported/updated', count: results.length, projects: results });
  } catch (error) {
